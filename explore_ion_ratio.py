...
def basic_imshow(ax,xfield,yfield1,yfield2,emis=False,plot_type='temp'):
	if emis:
...
ion1,ion2,ion3 = 'CIV','OVI','CIV' ##the first is CIII_977
count = 0
j = 0

for field in fields:
	fileout = key[j]+'_'+ion3+'_'+ion2+'_imshow_brightest.png'
	...
	for i in range(len(ax)):
		...
		if i < 3:
			...
			basic_imshow(ax[i],basic,ion_top,ion_btm,emis=True,plot_type=key[j])
			...
			#basic_plot(ax[i],basic,ion_top,ion_btm)
			basic_imshow(ax[i],basic,ion_top,ion_btm,emis=False,plot_type=key[j])
