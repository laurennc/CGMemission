...
temperature_beg = '/u/10/l/lnc2115/vega/repos/CGMemission/bertone_frbs/final/basic/'
...
fileout = 'frb_covering_fraction.png'#_nozscaling.png'
...
for ion in ions:
	...
	while count < len(model_gqs):
		...
		zplot = np.append(zplot,znow[count])
		count = count + 1
...
ax[1].set_title('SB > 2')
ax[2].set_title('SB > 3')
ax[0].set_ylabel('log(SB) [photons s^-1 cm^-2 sr^-1]')
