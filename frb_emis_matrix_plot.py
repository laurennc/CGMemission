...
ions = ['SiIV','CIV','OVI']
#ions = ['SiIV','CIII','OVI','HAlpha','CIII_977','CIV','MgII','SiII','SiIII_1207','SiIII_1883']
redshift_key = 'z0'
znow = 0
...
for ion in ions:
	fileout = 'frb_profile_matrix_'+ion+'_'+redshift_key+'_nointerp_zscaled_Zfixed_500kpc_HI.png'
	xlen,ylen = 3,6
	...
	while i < len(ax):
		if yes < xlen:
			...
			HIfile = HI_beg+'g1q1'+'/frbx_1kpc_500kpc_z02_HIdens.cpkl'
			HIfrb = cPickle.load(open(HIfile,'rb'))
			HIfrb = np.log10(HIfrb)

			if i in last:
				plot_frb(modelnames[2],ax[i],z=znow,include_colorbar=True)
				add_HI_contour(ax[i],HIfrb,ncontours)
			else:
				plot_frb(modelnames[2],ax[i],z=znow)
				add_HI_contour(ax[i],HIfrb,ncontours)

			full_scatter_plot(modelnames,ion,ax[i+xlen],res_keys[count],max_r,percentile,znow)
			yes  = yes + 1
			count = count + 1
		else:
			no = no + 1
